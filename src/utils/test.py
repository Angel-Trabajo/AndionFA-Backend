...
from src.db.query import get_node_by_id
...
def make_test(symbol, list_id):
    ...
    try:
        while True:
            ...
            decision = ''
            for node, df_indicadores in zip(list_nodes, list_df):
                try:
...
if __name__ == "__main__":
    with open('config/config_test/config_test.json', 'r', encoding='utf8') as file:
        data = json.load(file)
    make_test(data["symbol"], data["list_id"])
