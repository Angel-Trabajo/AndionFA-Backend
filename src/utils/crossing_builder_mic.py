    ...
    for nodo, mask_os, mask_is in zip(list_nodos, masks_os, masks_is):
        ...
        prev_os = (
            ...
            db_query.promedio_correct_percentage(
                f'crossing_{principal_symbol}_dbs/{list_symbol[cont-1]}', action, 'os'
            )
        )

        if not (prev_os + min_sri <= porcentaje_os <= prev_os + max_sri):
            continue
        ...
        prev_is = (
            ...
            if cont == 0 else
            db_query.promedio_correct_percentage(
                f'crossing_{principal_symbol}_dbs/{list_symbol[cont-1]}', action, 'is'
            )
        )

        if not (prev_is + min_sri <= porcentaje_is <= prev_is + max_sri):
            continue
