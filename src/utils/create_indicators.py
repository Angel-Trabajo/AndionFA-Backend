...
def _get_data_mt5(symbol, timeframe, start, end, folder):
    ...
    if folder == 'extrac':
        df.to_csv('output/is_os/is.csv', index=False)
    else:
        df.to_csv('output/is_os/os.csv', index=False)
...
def _generate_files(indicator_file, pos, symbol, start, end, timeframe, folder):
    ...
    if folder == 'extrac':
        df = pd.read_csv('output/is_os/is.csv')
    else:
        df = pd.read_csv('output/is_os/os.csv')
