...
from src.db.query import get_node_by_id, get_nodes
        ...
        while True:
            ...
            if cont == 1:
                for node in list_nodes:
                    ...
                    list_back_symbol= list_symbol[:list_symbol.index(symbol)+1]  
                    break
                list_files = os.listdir('output/extrac')
                list_files = [fi.split('_')[0] for fi in list_files]
                
                for symbol_test in list_back_symbol:
                    os.makedirs(f'output/data_test/{symbol_test}', exist_ok=True)
                    df = obtener_historicos_ultimo_anio(fecha_dt, symbol_test, list_timeframes[0])
                    for file in list_files:
                        df_indicadores = generate_files(f'{file}.csv', df) 
                        df_indicadores.to_csv(f'output/data_test/{symbol_test}/{file}_{symbol_test}.csv', index=False)
                list_back_symbol = list_back_symbol[::-1]
                 
                for symbol_test in list_back_symbol:
                    if symbol_test == prin_symbol:
                        nodes = get_nodes(symbol_test)
                        nodes = [node for node in nodes if node[1] == algoritmo]
                    else:
                        nodes = get_nodes(f'crossing_{prin_symbol}_dbs/{symbol_test}')
                        nodes = [node for node in nodes if node[1] == algoritmo]
                    list_objes_nodes.append({f'{symbol_test}': nodes})
                    
                

                for i, symbol_test in enumerate(list_back_symbol):
                    nodes = list_objes_nodes[i][symbol_test]
                    for node in nodes:
                        # Cada nodo apunta a un CSV distinto
                        fil = node[2].split('_')[0]
                        path = f'output/data_test/{symbol_test}/{fil}_{symbol_test}.csv'

                        if path not in cache_dfs:
                            # Solo se carga la primera vez
                            df_indicadores = pd.read_csv(path)
                            df_indicadores['time'] = pd.to_datetime(df_indicadores['time'])
                            df_indicadores.set_index('time', inplace=True)  # index rápido por fecha
                            cache_dfs[path] = df_indicadores    
            ...
            for i, symbol_test in enumerate(list_back_symbol):
                ...
                for node in nodes:
                    ...
                    df_indicadores = cache_dfs[path]  # ya en memoria

                    try:
                        # Buscar fila inmediatamente anterior a la fecha
                        indicador_row = df_indicadores.loc[:fecha_dt].iloc[-2]
                    except Exception:
                        print(f"[Python] No se encontró la fecha {fecha_dt} en {path}")
                        break

                    # node[3] cambia por nodo → debe evaluarse aquí
                    conditions = ast.literal_eval(node[3])
...
if __name__ == "__main__":
    ...
    eliminar_ruta('output/data_test')         
